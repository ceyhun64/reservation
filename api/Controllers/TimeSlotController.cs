...
public class TimeSlotController : ControllerBase
{
    ...
    )
    {
        ...
        if (dto.StartTime >= dto.EndTime)
            return BadRequest(
                ApiResponse<TimeSlotResponseDto>.Fail("Bitiş zamanı başlangıçtan büyük olmalıdır.")
            );

        if (dto.StartTime < DateTime.UtcNow)
            return BadRequest(
                ApiResponse<TimeSlotResponseDto>.Fail("Geçmiş tarihli slot eklenemez.")
            );
        ...
        if (conflict)
            return BadRequest(
                ApiResponse<TimeSlotResponseDto>.Fail("Bu zaman diliminde çakışan slot var.")
            );
        ...
        return Ok(
            ApiResponse<TimeSlotResponseDto>.Ok(
    ...
    )
    {
        ...
        return Ok(
            ApiResponse<object>.Ok(
                new { Created = slots.Count },
                $"{slots.Count} slot oluşturuldu."
            )
        );
    }
    ...
    public async Task<ActionResult<ApiResponse<object>>> Block(int slotId)
    {
        ...
        if (slot is null)
            return NotFound(ApiResponse<object>.Fail("Slot bulunamadı."));
    ...
    private async Task EnsureOwnerAsync(int providerId)
    {
        ...
        var provider =
            await _db.Providers.FindAsync(providerId)
