...
public class ProviderController : ControllerBase
{
        ...
        [FromQuery] ProviderSearchDto filter)
    {
        ...
        if (filter.CategoryId.HasValue)
            query = query.Where(p => p.ProviderServices
                .Any(ps => ps.Service.CategoryId == filter.CategoryId ||
                           ps.Service.Category.ParentCategoryId == filter.CategoryId));

        if (!string.IsNullOrWhiteSpace(filter.City))
            query = query.Where(p => p.Business != null &&
                p.Business.City.ToLower().Contains(filter.City.ToLower()));

        if (filter.MaxPrice.HasValue)
            query = query.Where(p => p.ProviderServices
                .Any(ps => (ps.CustomPrice ?? ps.Service.Price) <= filter.MaxPrice));
        ...
        return Ok(ApiResponse<PagedResponse<ProviderResponseDto>>.Ok(new PagedResponse<ProviderResponseDto>
        {
            Items = items,
            TotalCount = total,
            Page = filter.Page,
    ...
    public async Task<ActionResult<ApiResponse<ProviderResponseDto>>> GetById(int id)
    {
        ...
        if (p is null) return NotFound(ApiResponse<ProviderResponseDto>.Fail("Provider bulunamadı."));
    ...
    public async Task<ActionResult<ApiResponse<ProviderResponseDto>>> Create(ProviderDto dto)
    {
        ...
        if (await _db.Providers.AnyAsync(p => p.UserId == userId))
            return BadRequest(ApiResponse<ProviderResponseDto>.Fail("Bu kullanıcının zaten bir provider profili var."));

        if (dto.BusinessId.HasValue)
        {
            var biz = await _db.Businesses.FindAsync(dto.BusinessId);
            if (biz is null) return BadRequest(ApiResponse<ProviderResponseDto>.Fail("İşletme bulunamadı."));
        }
    ...
    {
        var userId = GetUserId();
        var p = await _db.Providers.Include(p => p.User).Include(p => p.Business)
                              .FirstOrDefaultAsync(p => p.Id == id);

        if (p is null) return NotFound(ApiResponse<ProviderResponseDto>.Fail("Provider bulunamadı."));
        if (p.UserId != userId && !User.IsInRole("Admin")) return Forbid();

        p.Title = dto.Title; p.Bio = dto.Bio;
        p.BusinessId = dto.BusinessId; p.AcceptsOnlineBooking = dto.AcceptsOnlineBooking;
        ...
        [FromBody] AddProviderServiceDto dto)
    {
        ...
        if (p is null) return NotFound(ApiResponse<object>.Fail("Provider bulunamadı."));
        if (p.UserId != userId && !User.IsInRole("Admin")) return Forbid();

        var service = await _db.Services.FindAsync(dto.ServiceId);
        if (service is null) return NotFound(ApiResponse<object>.Fail("Hizmet bulunamadı."));

        if (await _db.ProviderServices.AnyAsync(ps => ps.ProviderId == id && ps.ServiceId == dto.ServiceId))
    ...
    public async Task<ActionResult<ApiResponse<List<object>>>> GetServices(int id)
    {
        var services = await _db.ProviderServices
            .Include(ps => ps.Service).ThenInclude(s => s.Category)
    ...
    private static ProviderResponseDto ToDto(Provider p) =>
        new(p.Id, p.UserId, p.User?.FullName ?? "",
            p.BusinessId, p.Business?.Name,
            p.Title, p.Bio, p.ProfileImageUrl,
            p.AverageRating, p.TotalReviews, p.AcceptsOnlineBooking);
}

public record AddProviderServiceDto(int ServiceId, decimal? CustomPrice, int? CustomDurationMinutes);