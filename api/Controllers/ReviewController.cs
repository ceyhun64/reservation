...
public class ReviewController : ControllerBase
{
    ...
    )
    {
        ...
        return Ok(
            ApiResponse<PagedResponse<ReviewResponseDto>>.Ok(
                new PagedResponse<ReviewResponseDto>
                {
                    Items = items,
                    TotalCount = total,
                    Page = page,
    ...
    public async Task<ActionResult<ApiResponse<ReviewResponseDto>>> Create(ReviewCreateDto dto)
    {
        ...
        var appt = await _db
            .Appointments.Include(a => a.Review)
            .FirstOrDefaultAsync(a => a.Id == dto.AppointmentId);

        if (appt is null)
            return NotFound(ApiResponse<ReviewResponseDto>.Fail("Randevu bulunamadı."));
        if (appt.ReceiverId != userId)
            return Forbid();
        if (appt.Status != AppointmentStatus.Completed)
            return BadRequest(
                ApiResponse<ReviewResponseDto>.Fail(
                    "Sadece tamamlanan randevular değerlendirilebilir."
                )
            );
        if (appt.Review is not null)
            return BadRequest(
                ApiResponse<ReviewResponseDto>.Fail("Bu randevu için zaten değerlendirme yapılmış.")
            );
    ...
    public async Task<ActionResult<ApiResponse<ReviewResponseDto>>> Reply(
        int id,
        ReviewReplyDto dto
    )
    {
        var userId = GetUserId();
        var review = await _db
            .Reviews.Include(r => r.Author)
            .Include(r => r.Provider!.User)
            .FirstOrDefaultAsync(r => r.Id == id);

        if (review is null)
            return NotFound(ApiResponse<ReviewResponseDto>.Fail("Değerlendirme bulunamadı."));
        if (review.Provider.UserId != userId && !User.IsInRole("Admin"))
            return Forbid();
    ...
    public async Task<ActionResult<ApiResponse<object>>> Hide(int id)
    {
        ...
        if (review is null)
            return NotFound(ApiResponse<object>.Fail("Değerlendirme bulunamadı."));
    ...
    private async Task UpdateProviderRating(int providerId)
    {
        ...
        var reviews = await _db
            .Reviews.Where(r => r.ProviderId == providerId && r.IsVisible)
            .ToListAsync();
